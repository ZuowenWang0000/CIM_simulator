...
def quantize_and_evaluate(cfg, visualize=None, savefig=False):
    ...
    torch.quantization.prepare(quantized_encoder, inplace=False)

    # Use training data for calibration.
    print("calibrate model")
    # calibrate_model(model=quantized_encoder, loader=trainloader, device=cpu_device)
    print("calibrated!")
    quantized_encoder = torch.quantization.convert(quantized_encoder, inplace=False)
    print("quantized")
    ...
    im_size = 128
    num_samples = 500
    fp32_cpu_inference_latency = measure_inference_latency(model=encoder, device=cpu_device, input_size=(1,1,im_size,im_size), num_samples=num_samples, num_warmups=100)
    int8_cpu_inference_latency = measure_inference_latency(model=quantized_encoder, device=cpu_device, input_size=(1,1,im_size,im_size), num_samples=num_samples, num_warmups=100)
    # int8_jit_cpu_inference_latency = measure_inference_latency(model=quantized_jit_encoder, device=cpu_device, input_size=(1,1,im_size,im_size), num_samples=num_samples, num_warmups=100)
    fp32_gpu_inference_latency = measure_inference_latency(model=encoder, device=cuda_device, input_size=(1,1,im_size,im_size), num_samples=num_samples, num_warmups=100)
    int8_gpu_inference_latency = measure_inference_latency(model=quantized_encoder, device=cuda_device, input_size=(1,1,im_size,im_size), num_samples=num_samples, num_warmups=100)
